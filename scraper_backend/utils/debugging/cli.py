...
Usage:
    # Validate a config
    python -m scraper_backend.utils.debugging.cli validate path/to/config.yaml

    # Validate all configs in a directory
    python -m scraper_backend.utils.debugging.cli validate-all path/to/configs/

    # Test a selector against a URL
    python -m scraper_backend.utils.debugging.cli test-selector "#productTitle" --url "https://amazon.com/dp/..."

    # Test all selectors from a config
    python -m scraper_backend.utils.debugging.cli test-config path/to/config.yaml --sku "035585499741"

    # Debug a workflow step by step
    python -m scraper_backend.utils.debugging.cli debug path/to/config.yaml --sku "035585499741"
"""
